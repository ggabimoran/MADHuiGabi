...
Permutons les lignes de la matrice $X$ à $200$ lignes et $50$ colonnes et visualisons la matrice ainsi obtenue. Procédons à la même permutation pour la matrice $Z$ des variables latentes.
...
M_Step<-function(X_,T_){
  ...
  new_mu <- t(apply(new_mut,MARGIN=1,function(k){colSums(t(t(X)*c(T[,k])))/colSums(T)[k]}))
  #Calcul des pi_k
  new_pi <- colSums(T_)/n
}
```
