...
#working on showing z==t
newZ<-E_Step(X,M,pi)
newZcluster<-apply(newZ,which.max,MARGIN = 1)
basecluster<-res.kmeans$cluster
library(combinat)
cluster_permutations <- permn(1:3)
for (i in 1:length(cluster_permutations)){
  basecluster<-res.kmeans$cluster
  basecluster[basecluster==1]<-cluster_permutations[[i]][1]
  basecluster[basecluster==2]<-cluster_permutations[[i]][2]
  basecluster[basecluster==3]<-cluster_permutations[[i]][3]
  print(sum(basecluster==newZcluster))
}
```
